    ...
    def shift(self, n: int, mu: int):
        ...
        while n > 0:
            ...
            if rank == source and rank == dest:
                ...
                sendbuf_host = numpy.ascontiguousarray(sendbuf_host) #! modified by Jinchen He, 2025-11-20
...
class FullField(BaseField, Generic[Field]):
    ...
    def shift(self, n: int, mu: int):
        ...
        import dpnp #! modified by Jinchen He, 2025-11-19
        ...
        while n > 0:
            if mu == 0 and n == 1:
                ...
                eo_np = numpy.sum(numpy.indices((2, *self.latt_info.size[1:][::-1])), axis=0).reshape(-1) % 2
                
                eo = arrayDevice(eo_np, location) #! modified by Jinchen He, 2025-11-19
                
                even = eo == 0
                odd = eo == 1
                
                even_idx = dpnp.nonzero(even)[0] #! modified by Jinchen He, 2025-11-19
                odd_idx = dpnp.nonzero(odd)[0] #! modified by Jinchen He, 2025-11-19
                
                if dir == 1:
                    # sendbuf = right_flat[even, 0] 
                    sendbuf = right_flat[even_idx][:, 0] #! modified by Jinchen He, 2025-11-19
                    if rank == source and rank == dest:
                        pass
                    else:
                        sendbuf_host = arrayHostCopy(sendbuf, location)
                    ...
                    right_tmp = right_flat[odd_idx].reshape( #! modified by Jinchen He, 2025-11-19
                    ...
                    right_tmp = right_flat[even_idx][:, 1:].reshape( #! modified by Jinchen He, 2025-11-19
                    ...
                    sendbuf = right_flat[odd_idx][:, -1] #! modified by Jinchen He, 2025-11-19
                    if rank == source and rank == dest:
                        pass
                    else:
                        sendbuf_host = arrayHostCopy(sendbuf, location)
                    ...
                    right_tmp = right_flat[even_idx].reshape( #! modified by Jinchen He, 2025-11-19
                        2, prod(self.latt_info.size[1:]) // 2, self.latt_info.size[0] // 2, -1
                    )
