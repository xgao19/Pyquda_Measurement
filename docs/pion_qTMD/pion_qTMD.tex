...
\begin{align}
  ...
  \texttt{prop\_b} &\leftarrow \text{boosted\_smearing}(\texttt{prop\_b},
                      w, \texttt{neg\_boost}).
\end{align}
Boosted smearing is a gauge-covariant Gaussian convolution in momentum space
with an injected momentum shift: the fermion field is Fourier-transformed
(assuming identity gauge, i.e., already fixed to a smooth gauge), multiplied by
a momentum-shifted Gaussian kernel $\exp\bigl(-\frac{|\mathbf{r}|^2}{2w^2} + i
\mathbf{k}_{\rm boost}\cdot\mathbf{r}\bigr)$, and inverse-transformed back to
position space.
...
\begin{equation}
  K(\mathbf{r}) = \exp\!\left(
    -\frac{|\mathbf{r}|^2}{2w^2}
    + i\,\mathbf{k}_{\rm boost}\cdot\mathbf{r}
  \right),
\end{equation}
