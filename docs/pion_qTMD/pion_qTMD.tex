...
The default source gamma is \texttt{fixed\_g5}, which sets $\Gamma_{\rm src}=
\gamma_5$ for all sink channels, corresponding to the standard pseudoscalar
pion interpolator $\bar\psi \gamma_5 \psi$. Three source modes are supported in
the code (\texttt{\_source\_gamma\_stack}):
\begin{itemize}[nosep]
  \item \texttt{fixed\_g5}: All source gammas are $\gamma_5$.
  \item \texttt{same\_as\_sink}: $\Gamma_{\rm src} = \Gamma_{\rm sink}$.
...
Before contraction, both the forward propagator $\texttt{prop\_f}$ and the
backward propagator $\texttt{prop\_b}$ are boosted-smeared at the sink using
separate boost parameters \texttt{pos\_boost} and \texttt{neg\_boost}:
\begin{align}
  \texttt{prop\_f} &\leftarrow \text{boosted\_smearing}(\texttt{prop\_f},
                      w, \texttt{pos\_boost}), \\
  \texttt{prop\_b} &\leftarrow \text{boosted\_smearing}(\texttt{prop\_b},
                      w, \texttt{neg\_boost}).
\end{align}
...
$\texttt{prop}$ (already boosted-smeared with \texttt{neg\_boost}), the source
...
\begin{equation}
  \eta_{\rm seq}^{SS}
  =
  \mathcal{S}_{w,\mathbf{k}_{\rm pos}}
  \left[
    \delta_{t_y,\, t_{\rm sink}}\;
    \Phi_{p_f}(\mathbf{y}-\mathbf{x}_0)\;
    \Gamma_{\rm seq}\,
    S_q^{S\to S,\,{\rm neg}}(\mathbf{y},t_{\rm sink};\mathbf{x}_0,0)
  \right].
  \label{eq:qtmd_eta_seq_smear}
\end{equation}
The input line \(S_q^{S\to S,\,{\rm neg}}\) already carries the spectator-line
sink smearing with \texttt{neg\_boost}.  The outer
\(\mathcal{S}_{w,\mathbf{k}_{\rm pos}}\) supplies the active-line sink smearing
associated with the sequential propagator.  This extra smearing is needed for a
consistent smeared-smeared three-point function and matches the proton
fixed-sink sequential-source pattern.
...
set as in Sec.~\ref{sec:twopt}), while $\Gamma_{\rm src}$ is typically
\texttt{fixed\_g5} and $\Gamma_{\rm sink}$ is set by the application (e.g.,
$\gamma_5$ for the pseudoscalar pion).

The einsum contraction pattern in \verb|_contract_qTMD_one_shift| mirrors the
two-point function:
\begin{align}
  ...
  (\Gamma_{\rm src})_{l j}.
\end{align}
Here $\texttt{shifted\_prop}$ is the forward propagator with the nonlocal
displacement operator already applied (see Sec.~\ref{sec:cg_tmd_op}).
...
The incremental shift operation in \verb|create_fw_prop_TMD_CG| computes
\begin{equation}
...
\verb|create_fw_prop_TMD_CG| method, which applies a lattice shift in the $z$
...
The implementation in \verb|create_fw_prop_PDF_GI| applies the gauge-covariant
derivative \texttt{gauge.pure\_gauge.covDev(fermion, dir)} spin-color by
spin-color:
\begin{itemize}[nosep]
...
The method \verb|create_TMD_Wilsonline_index_list_CG| returns two lists:
...
After enumeration, \verb|_reorder_wilson_indices| reorders each list to
minimize the maximum absolute difference in $(b_T, b_z)$ between adjacent
...
The method \verb|create_PDF_Wilsonline_index_list| returns a single list
(because $b_T = 0$ always):
\begin{verbatim}
for bz in 0 .. b_z:
    add [0, bz, 0, 0]
...
\begin{itemize}[nosep]
  \item \texttt{eta}: staple length parameter (list, typically \texttt{[0]}).
  \item \texttt{b\_z}, \texttt{b\_T}: maximum longitudinal and transverse
        displacements.
  \item \texttt{pf}: sink boost momentum $[p_x, p_y, p_z, p_t]$.
  \item \texttt{qext}, \texttt{qext\_PDF}: insertion momentum lists for qTMD
  ...
  \item \texttt{t\_insert}: sink-source separation $t_{\rm sep}$.
  \item \texttt{save\_propagators}: whether to save propagators to disk.
\end{itemize}
...
Workflow:
\begin{enumerate}[nosep]
  \item Boosted-smeared \texttt{prop\_f} (with \texttt{pos\_boost}) and
        \texttt{prop\_b} (with \texttt{neg\_boost}).
  \item Build sink gamma stack (\verb|_gamma_stack|) and source gamma stack
        (\verb|_source_gamma_stack|).
  \item Construct antiquark line via \verb|_meson_backward_line| (einsum with
        $\gamma_5$ front and back).
  \item Loop over sink Gamma channels, attach one Gamma to the backward line,
        contract it with the forward propagator and matching source Gamma,
        and immediately Fourier-project the resulting scalar field.
  \item Gather across MPI ranks (\texttt{core.gatherLattice}).
...
\begin{enumerate}[nosep]
  ...
  \item For direction 0 ($x$): iterate over \texttt{W\_index\_list\_dir0},
        incrementally shifting \texttt{tmd\_forward\_prop\_dir0} using
        \verb|create_fw_prop_TMD_CG| and contracting at each step via
        \verb|_contract_qTMD_one_shift|.
...
\begin{itemize}[nosep]
  \item Uses a single \texttt{W\_index\_list} (no transverse direction
        separation).
  \item Resets the forward propagator to the original copy whenever $b_z$
        returns to $0$ (handling the split between positive and negative
        branches).
  \item Calls \verb|create_fw_prop_PDF_GI| when \texttt{gauge\_invariant=True}
        or \verb|create_fw_prop_TMD_CG| when \texttt{gauge\_invariant=False}.
\end{itemize}

\subsection{\texttt{\_contract\_qTMD\_one\_shift}}
...
\subsection{\texttt{create\_fw\_prop\_TMD\_CG}}
...
\subsection{\texttt{create\_fw\_prop\_PDF\_GI}}
...
transpose is critical: \verb|save_qTMD_pion_hdf5_noRoll| (which delegates to
\verb|save_qTMD_proton_hdf5_noRoll|) expects the momentum dimension to precede
the gamma dimension.  Without this transpose, a single-gamma HDF5 file could be
written with a momentum list interpreted as gamma labels, producing
...
\texttt{.src<SRC>.sink<SINK>.<operator-gamma>}.  Their root attributes store
the same source and sink labels together with \texttt{operator\_gamma}.  The
...
where:
\begin{itemize}[nosep]
  \item \texttt{SS}: sequential-source group.
  \item \texttt{<gamma>}: one of the 16 gamma labels
        (\texttt{5}, \texttt{T}, \texttt{T5}, \dots, \texttt{SZT}).
  \item \texttt{PX..PY..PZ..}: insertion momentum components.
  \item \texttt{b\_X} or \texttt{b\_Y}: transverse displacement direction.
  \item \texttt{eta0}: staple length parameter (currently 0).
  \item \texttt{bT<bT>}: transverse displacement magnitude.
  \item \texttt{bz<bz>}: longitudinal displacement (dataset with time-series
        data).
\end{itemize}

For the PDF path ($b_T = 0$), the datasets appear under \texttt{b\_X/eta0/bT0/}
regardless of the lack of transverse displacement, maintaining structural
compatibility with the qTMD layout.
...
In the current pion qTMD workflow, both \texttt{pos\_boost} and
\texttt{neg\_boost} are set to $\mathbf{0}$, and the pion boost is provided
solely by the sink momentum $p_f$ encoded in the sequential source.  The
parameters are retained for future workflows that may use non-zero boost
smearing to improve overlap with boosted pion states.
...
\begin{tabular}{c|c|l}
\hline
\textbf{Index} & \textbf{Label} & \textbf{Description} \\ \hline
0  & \texttt{5}   & $\gamma_5$ \\
...
9  & \texttt{I}   & Identity \\
10 & \texttt{SXT} & $\Sigma_{XT} = \frac{i}{2}[\gamma_X,\gamma_T]$ \\
11 & \texttt{SXY} & $\Sigma_{XY}$ \\
12 & \texttt{SXZ} & $\Sigma_{XZ}$ \\
13 & \texttt{SYT} & $\Sigma_{YT}$ \\
14 & \texttt{SYZ} & $\Sigma_{YZ}$ \\
15 & \texttt{SZT} & $\Sigma_{ZT}$ \\
\hline
\end{tabular}
\end{center}
...
coordinate-shift qTMD and a gauge-invariant staple qTMD.  The old
\texttt{application/pion\_TMD\_CG/perlmutter} workflow is intentionally kept as
the already-tested CG-only path.  The newer
...
The implementation uses the same tested helper convention as the disconnected
qTMD one-point code: first build reusable gauge-only staple transporters, then
apply each transporter to the endpoint-shifted forward propagator.  In the pion
source file this is exposed through
\begin{itemize}[nosep]
  \item \texttt{create\_TMD\_Wilsonline\_index\_list\_GI()},
  \item \texttt{create\_fw\_prop\_TMD\_GI(...)},
  \item \texttt{contract\_qTMD\_GI(...)}.
\end{itemize}
