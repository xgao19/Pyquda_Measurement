...
position $x_0$.  The phase generator returns
$e^{+i\mathbf{p}_{\rm arg}\cdot(\mathbf{x}-\mathbf{x}_0)}$.  For the
two-point function, negated three-momenta $[-p_x,-p_y,-p_z]$ are passed as
$\mathbf p_{\rm arg}$, so that
\begin{equation}
...
$\mathbf{q}$ (insertion) with
$\mathbf{q}=\mathbf{p}_f-\mathbf{p}_i$, equivalently
...
\mathbf{r}}$ is generated by \texttt{MomentumPhase.getPhases} from the
argument $+\mathbf p_f$.  Unlike the two-point projection, no negation is
applied at this stage.  The time slice restriction is
...
\begin{equation}
  ...
  \label{eq:seq_anti}
\end{equation}
The complex conjugation converts the source phase
$e^{+i\mathbf p_f\cdot(\mathbf y-\mathbf x_0)}$ into the effective fixed-sink
projector $e^{-i\mathbf p_f\cdot(\mathbf y-\mathbf x_0)}$ in the full
