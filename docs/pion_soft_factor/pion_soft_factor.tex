...
\paragraph{Global shift under MPI decomposition.}
Every nonzero transverse displacement is applied to the complete PyQUDA
\texttt{LatticePropagator} through \texttt{prop.shift}. This invokes the halo
communication required when the transverse direction is MPI decomposed.
Rolling a rank-local lexicographic array would instead wrap at the local
subdomain boundary and produce a decomposition-dependent four-point function.
% ===========================================================================

\subsection{Coordinate-Gauge Shifts}

The transverse displacement $\mathbf{b}$ is implemented by a global
propagator shift in the coordinate gauge (CG). No explicit gauge link is
inserted between the shifted quark fields. This is consistent with the CG
convention used in the pion qTMD workflow.
...
\begin{verbatim}
    shifted_prop = prop.shift(bT, bT_dir)
\end{verbatim}

The PyQUDA field operation owns the conversion from physics direction to field
layout and communicates across MPI subdomain boundaries. The rank-local
lexicographic array is materialized only after the shift.
