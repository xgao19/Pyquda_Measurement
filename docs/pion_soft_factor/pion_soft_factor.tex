...
\walltag{pion\_src}  & Dictionary of source interpolators (default: \walltag{Z5-X5}) \\
\walltag{pion\_sink} & Dictionary of sink interpolators (default: same as src) \\
\walltag{Gamma1}     & Dictionary of $\Gamma_{1}$ insertions (default: soft-factor set) \\
\walltag{Gamma2}     & Dictionary of $\Gamma_{2}$ insertions (default: soft-factor set) \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Backend abstraction}: All \einsum\ calls are dispatched through
        ...
        or PyTorch backends without modification.
\end{enumerate}
