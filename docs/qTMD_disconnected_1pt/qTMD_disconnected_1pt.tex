...
Production uses base/HP-interval shards by default.  One complete base
estimator is
\begin{equation}
...
\end{center}
The old source-position helper remains available for existing files and APIs;
the full-volume loop itself has no hadron source position.  The writer stores:
\begin{equation}
...
The current S8T32 Perlmutter login-node check used one matched counter-based
stochastic vector and found exact agreement:
\begin{equation}
  \max\left|
  L^{\mathrm{GI\_PDF}}
  -
...
The reference code-side helper is
...
For production, the default \(\code{QTMD\_1PT\_GI\_STAPLE\_MODE=link\_cache}\)
first builds gauge-only staple transporters with
...
configuration to reuse the same staple links.  The debug mode
\(\code{QTMD\_1PT\_GI\_STAPLE\_MODE=direct\_covdev}\) keeps the older
source-by-source reference implementation.
