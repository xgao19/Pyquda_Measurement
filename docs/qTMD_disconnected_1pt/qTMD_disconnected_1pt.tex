...
a staple Wilson line.  A convenient target convention is
\begin{equation}
...
\begin{equation}
  ...
  x+\eta\hat{z}
  \rightarrow
  x+\eta\hat{z}+b_T\hat{e}_{\perp}
  \rightarrow
  x+b_z\hat{z}+b_T\hat{e}_{\perp}.
\end{equation}
The last segment has signed length \(b_z-\eta\).  With \(b_T=0\) and
\(\eta=b_z\), this reduces to the straight \(\code{GI\_PDF}\) Wilson line.  At
\(b_T=b_z=\eta=0\), it reduces to the local operator.
...
\begin{enumerate}
  ...
  \item Validate the limits:
        \(\code{GI\_qTMD}(b_T=0,\eta=b_z)=\code{GI\_PDF}(b_z)\),
...
local, straight-PDF, and generic three-segment path conventions before the
covariant-shift implementation is wired into production contractions.
