...
In the implementation, the left derivative is applied by:
\begin{enumerate}
  \item Wrapping \texttt{seq\_data} as a \texttt{LatticePropagator};
  \item Applying the symmetric covariant derivative $\frac{1}{2}(D_{+\mu}-D_{-\mu})$
        to this propagator (the derivative naturally acts on the ``source'' index,
        which is the insertion-side index of the sequential propagator);
  \item Extracting the data from the result.
\end{enumerate}
...
\begin{enumerate}
  ...
  \item \textbf{Covariant derivative on sequential data.}
        In the meson case, the left derivative is applied to \texttt{dst2} through
        \texttt{\_left\_covdev\_dst2\_from\_prop}.  In the proton case, the
        covariant derivative is applied directly to \texttt{seq\_data} through
        \texttt{\_covdev\_sym\_seq}, which wraps the data as a
        \texttt{LatticePropagator} and applies the symmetric derivative.
...
The \texttt{save\_emt\_quark\_3pt\_hdf5} function saves the following datasets:
...
\midrule
\texttt{C2} & \texttt{[Nt]} & Zero-momentum, selected-gamma two-point function \\
\texttt{C3\_chi} & \texttt{[Nflavor, Npol, Nts, Nsteps+1, Nq, Nt]}
                  & Scalar diagnostic three-point function \\
\texttt{C3\_Tmunu} & \texttt{[Nflavor, Npol, Nts, Nsteps+1, Nq, 4, 4, Nt]}
...
and the selected two-point momentum used for normalization.
...
The renormalization of the flowed quark EMT requires the ringed-fermion kinetic
normalization.  At zero momentum, the diagonal quark EMT trace provides
\begin{equation}
  ...
  = -\frac{1}{2}\sum_{\mathbf{x}}
    ...
    \big\rangle,
\end{equation}
up to the lattice derivative convention.  This quantity is accessible from the
stochastic quark 1pt HDF5 output as the sum of the diagonal components
\texttt{avg/Tmunu/T11} through \texttt{T44} at the $\momtrans = 0$ momentum index.
