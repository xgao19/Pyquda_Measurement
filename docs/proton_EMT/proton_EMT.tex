...
\texttt{PX<px>PY<py>PZ<pz>dt<tsep>}.  For example, a zero-momentum sink with
source--sink separation nine carries the suffix
\texttt{PX0PY0PZ0dt9.h5}.  This prevents runs with different final momenta or
source--sink separations from overwriting one another and follows the nucleon
TMD output convention.
...
\code{T22}, \code{T33}, and \code{T44}.  The average over \(\tau\) and gauge
configurations should be performed in analysis, not inside the contraction
kernel.  The unflowed step \(t=0\) must not be used for ringed normalization
because the formula contains \(t^{-2}\).
