...
\qquad q=p_f-p_i,\qquad
Q^2 = -q^2 = -(p_f - p_i)^2 \ge 0 .
\label{eq:emff_def}
\end{equation}
...
Eq.~\eqref{eq:C2_trace}.  After workflow-specific sink smearing, it calls the
same shared pion C2 kernel used by pion EMT, qTMDWF, and qTMD.  The procedure
inside that kernel is:
...
Here \texttt{MomentumPhase} returns
\(e^{+i\mathbf{p}_{\rm arg}\cdot(\mathbf{x}-\mathbf{x}_0)}\).  For the
two-point function the caller supplies
\(\mathbf{p}_{\rm arg}=-\mathbf{p}_q\), producing the effective projector
\(\texttt{phases}_{q,w,t,z,y,x}
=e^{-i\mathbf{p}_q\cdot(\mathbf{x}-\mathbf{x}_0)}\) for each momentum mode
\(q\) in \texttt{p\_2pt}.  The sum over spatial indices
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Momentum phase:} Multiply by
        ...
        spatial lattice sites at the sink time slice.  This is the direct
        output of \texttt{MomentumPhase} for the argument \(+\mathbf p_f\).
        \begin{verbatim}
  ...
  final_term = xp.einsum("wtzyxijfc, ik -> wtzyxjkcf",
        ...
        \end{verbatim}
        The complex conjugation converts the \(+\mathbf p_f\) source phase
        into the effective sink projector
...
The root attributes record \texttt{src\_interpolator=<SRC>} and
\texttt{sink\_interpolator=all\_16\_gamma\_scan}.
...
The file contains all 16 current gamma channels.  Its root attributes record
\texttt{src\_interpolator}, \texttt{sink\_interpolator}, and
\texttt{current\_gamma\_basis=all\_16}.  For a multi-source-interpolator run,
the complete sorted source list and the sink interpolator enter the sample-log
identity.  The \texttt{<sm\_tag>} itself describes only preprocessing,
smearing, and boost parameters.

\paragraph{Lightweight resume.}
Each \(t_{\rm sep}\) has a text sample log.  Rank~0 reads all logs once and
broadcasts their exact completed-source sets.  If every requested
\(t_{\rm sep}\) is complete, the source is skipped before inversion; otherwise
only the missing separations are contracted and written.  A completion line is
durably appended only after every requested fixed-source-Gamma file for that
source and separation has closed.  The log is the sole resume state and does
not probe HDF5 files that may already have been transferred.
The production interface names the requested list
\texttt{t\_separations}; for example,
\texttt{--t\_separations 2,3,4}.  Each loop iteration passes one scalar
\(t_{\rm sep}\) to sequential-source construction.
The current log name assumes fixed run flags, \(q_{\max}\), momenta, source
Gamma list, and separation list.  A changed physical grid must use a new
data/setup identity, and concurrent writers to one log are unsupported.
...
\(\texttt{<sm\_tag>}\)     & Smearing tag (HYP steps, width,
                          boost vectors) \\
\(\texttt{<SRC>}\)        & Source interpolator label \\
\(\texttt{<SINK>}\)       & Sink interpolator label \\
