...
at non-zero momentum, the code employs \emph{gauge-covariant Gaussian smearing}
with momentum injection (boosted smearing).
...
K(\mathbf{r}; w, \mathbf{k})
  = \exp\!\Big(-\frac{\mathbf{r}^2}{2w^2}\Big) \,
    \exp\!\Big(2\pi i \, \frac{\mathbf{k}\cdot\mathbf{r}}{L}\Big) ,
\label{eq:kernel}
\end{equation}

where \(w\) is the Gaussian width (in lattice units), \(\mathbf{k}\) is the
injected momentum (integer vector), and \(L\) is the spatial lattice extent.
The first factor is the standard gauge-covariant Gaussian; the second factor
imparts momentum to the source or sink.
...
\begin{itemize}[nosep]
  ...
  \item \texttt{pyquda\_measurement\_utils/boosted\_smearing\_pyquda.py}
        --- Implements gauge-covariant Gaussian smearing with momentum
