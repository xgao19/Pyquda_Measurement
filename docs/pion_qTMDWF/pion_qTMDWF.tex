...
The mature implementation supports three source gamma modes, specified by the \texttt{src\_mode} parameter:
...
In contrast, the development version additionally supports an arbitrary specific gamma label (e.g., \texttt{"Z"}, \texttt{"X"}).
...
Both the mature and development modules use the same boosted smearing implementation from \texttt{boosted\_smearing\_pyquda.py}. The boosted smearing is a gauge-covariant Gaussian smearing with momentum injection, defined in the continuum as:
...
\begin{enumerate}
    ...
    \begin{equation}
        K(\mathbf{x}) = \exp\left(-\frac{x^2 + y^2 + z^2}{2w^2}\right) \,
        \exp\left(2\pi i \left( \frac{k_x}{L_x} x + \frac{k_y}{L_y} y + \frac{k_z}{L_z} z \right)\right).
    \end{equation}
...
It defines the class \texttt{pion\_TMDWF\_measurement} and the following module-level structures:
...
\begin{itemize}
    ...
    \item \texttt{G5}: The $\gamma_5$ matrix object, \texttt{gamma.gamma(15)}.
\end{itemize}
...
\begin{itemize}
    ...
    \item \texttt{src\_mode}: One of \texttt{"fixed\_g5"}, \texttt{"same\_as\_sink"}, \texttt{"dagger\_of\_sink"}.
\end{itemize}
...
\hline
Source gamma & 3 modes only: \texttt{fixed\_g5}, \texttt{same\_as\_sink}, \texttt{dagger\_of\_sink} & 4 modes: the 3 above plus any specific gamma label (e.g., \texttt{"Z"}) \\
\hline
