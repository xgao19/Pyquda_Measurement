...
\begin{itemize}
    ...
    \end{equation}
    \item The positive-boost forward propagator is held fixed as the spectator
    line. The negative-boost backward propagator is the active operator line.
    \item The CG qTMD operator displaces the backward active propagator in the
    transverse ($\hat{x}$ or $\hat{y}$) and longitudinal ($\hat{z}$)
    directions by integer lattice units.
...
The shared pion C2 kernel supports two source Gamma forms, selected by the
...
The \texttt{dagger\_of\_sink} mode pairs each sink channel with its own source
matrix.  Its output Gamma axis is therefore a paired sink/source axis, not a
scan of sink operators at one fixed source Gamma.  The qDA local C2 uses this
mode; qTMDWF production uses the explicit source label \texttt{"5"}.
...
In the coordinate-gauge (CG) approximation, the Wilson line is replaced by the identity, and the operator insertion reduces to a spatial displacement of the negative-boost backward propagator:
...
Unlike the fixed-sink connected pion qTMD/PDF workflow, qTMDWF is organized as
a two-propagator correlator and has no sequential active-line ambiguity.  For
unequal boosts it constructs and inverts the positive and negative source
lines independently.  The zero-boost application may copy the positive
propagator only because its two source-smearing kernels are identical.
...
Applies an incremental spatial shift to the backward active propagator for the CG qTMD path. The shift is computed as:

\begin{equation}
    B_- \leftarrow B_-.\texttt{shift}(\Delta b_T, \perp\text{-dir}).\texttt{shift}(\Delta b_z, z\text{-dir}),
\end{equation}
...
The application layer iterates over Wilson-line indices and transports the
backward propagator. qTMDWF and qDA then call the same shared
\texttt{contract\_pion\_gamma\_scan} kernel used by pion C2, qTMD, and EMFF.
Thus source-Gamma placement, sink-Gamma order, momentum projection, and MPI
gather are defined in one place.

\subsubsection{Memory-light platform Gamma scan}

The Frontier and Aurora applications call the shared
\texttt{contract\_pion\_gamma\_scan} helper for each displaced backward
propagator.  The contraction is deliberately ordered in two stages:
\begin{align}
 M_{kl}(x)
 &=
 \sum_{\mathrm{spin,color}}
 (\Gamma_{\mathrm{src}}\gamma_5)_{ij}
 S_b(x)^\dagger_{kj}\,S_f(x)_{li},
 \\
 C_g(\mathbf q,t)
 &=
 \sum_{\mathbf x}e^{i\mathbf q\cdot\mathbf x}
 \sum_{kl}M_{kl}(x)(\gamma_5\Gamma_g)_{kl}.
\end{align}
Consequently, the largest spin-bearing temporary has shape
\texttt{[local sites,4,4]}, and the largest Gamma-dependent field has shape
\texttt{[16,local sites]}.  The implementation must not first construct
\texttt{[16,local sites,4,4,3,3]}, which is sixteen full propagators and is
unnecessarily large.

For the Frontier script's $64^4$ lattice and MPI geometry
\texttt{2.2.2.4}, each rank owns $32\times32\times32\times16$ sites.  In
complex double precision, a Gamma-stacked propagator alone would occupy
approximately $18$~GiB per rank.  The two-stage ordering reduces the
Gamma-dependent field to approximately $128$~MiB per rank while leaving the
Gamma order, momentum phase, Wilson-index order, and HDF5 axes unchanged.
...
\hline
Source gamma & A canonical Gamma label or \texttt{dagger\_of\_sink} & Same shared modes and labels \\
\hline
...
%==========================================================================
\section{HDF5 Output}
The canonical application writes one rank-0 HDF5 file per source/operator
containing the complete 16-Gamma axis. The former per-rank, per-Gamma files are
not produced. A lightweight exact-line sample log is read before source
inversion; a source is recorded only after the C2 and full qTMDWF file close.
\label{sec:hdf5}
%==========================================================================
...
gauge.hypSmear(1, 0.75, 0.6, 0.3, -1)
    ...
    \begin{enumerate}
        ...
        \item Rank 0 saves one HDF5 file containing all 16 Gamma structures.
    \end{enumerate}
\end{enumerate}
...
\begin{enumerate}
    ...
    \item \textbf{Root-only I/O:} MPI reduction produces the complete Gamma
    axis on rank 0, which writes one serial HDF5 file.
\end{enumerate}
