...
interpolator \(\gsink\).  Here \texttt{MomentumPhase} receives
\(+\mathbf q\) directly, so that
\(\Phi_{\mathbf q}(\mathbf x;\mathbf x_0)
=e^{+i\mathbf q\cdot(\mathbf x-\mathbf x_0)}\); consequently
\(\mathbf q=\mathbf p_f-\mathbf p_i\) and
\(\mathbf p_i=\mathbf p_f-\mathbf q\).  The standard EMFF choice is the temporal vector
current,
\begin{equation}
  \Gamma = \gamma_T,
\end{equation}
