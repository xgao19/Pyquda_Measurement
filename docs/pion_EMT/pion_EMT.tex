...
\begin{tabular}{ll}
  ...
  \code{GluonEMT(EMTDisconnectedGluon1pt)} & alias for gluon 1pt \\
\end{tabular}
\end{center}
...
\begin{itemize}[nosep]
  ...
  \item \code{application/EMT\_meson/perlmutter/Pyquda\_EMT\_quark\_1pt.py} -- quark 1pt
  \item \code{application/EMT\_meson/perlmutter/Pyquda\_EMT\_gluon\_1pt.py} -- gluon 1pt
\end{itemize}
...
with $\sigma_{\mu\nu} = \frac{i}{2}[\gamma_\mu, \gamma_\nu]$. The ordering is fixed by \code{pyquda\_gammas\_order}:
...
The Dirac gamma matrices used in the contraction are $\gamma_1, \gamma_2, \gamma_3, \gamma_4$, indexed by \code{D\_GAMMA\_IDS = [1, 2, 4, 8]}. The index \code{mu=0} corresponds to $\gamma_1$ ($x$-direction in PyQUDA convention), \code{mu=1} to $\gamma_2$ ($y$), \code{mu=2} to $\gamma_3$ ($z$), and \code{mu=3} to $\gamma_4$ (time).
...
\begin{itemize}[nosep]
  \item \code{C2}: 1D array of length $N_t$ -- two-point function at the selected sink gamma and zero momentum, for normalization.
...
with \code{n = n\_zn} (typically $n = 2$, i.e., $\mathbb{Z}_2$ noise). Each spin-color component at each lattice site receives an independent random phase. The noise satisfies $\langle \xi(x) \rangle = 0$ and $\langle \xi(x) \xi^\dagger(y) \rangle = \delta_{xy}$.
...
\subsubsection{CHI diagnostics}

Two scalar diagnostics are computed to monitor the stochastic estimator:
...
\text{CHI}[0] is the stochastic estimate of $\Tr(D^{-1})$ and should average to the scalar vacuum condensate. \text{CHI}[1] is the noise normalization (for $\mathbb{Z}_2$ noise, $\xi^\dagger\xi = \text{constant}$ at each site).

In code:
\begin{verbatim}
  dot_xi_eta = contract("etzyxbc, etzyxbc -> etzyx",
                        xi.data.conj(), eta.data)
...
The file (written by \code{save\_emt\_quark\_1pt\_hdf5}) contains:

\begin{itemize}[nosep]
  \item \code{raw/Tmunu\_pervec}: 6D \code{[n\_vec, 4, 4, Nq, Nsteps+1, Nt]} -- per-noise-vector EMT.
  \item \code{raw/CHI\_pervec}: 5D \code{[n\_vec, 2, Nq, Nsteps+1, Nt]} -- per-noise-vector CHI.
  \item \code{avg/Tmunu/T11, T12, ..., T44}: averaged, volume-normalized tensor components (symmetric storage: only $\mu \leq \nu$).
  \item \code{avg/CHI}: 4D \code{[2, Nq, Nsteps+1, Nt]} -- averaged CHI.
...
The ringed fermion renormalization scheme \cite{Makino:2016} requires the flowed fermion kinetic expectation value. At zero momentum ($q = 0$), the diagonal sum of the quark EMT one-point provides precisely this quantity:

\begin{align}\label{eq:ringed_norm}
  \sum_{\mu=1}^4 T_{\mu\mu}^q(0, t)
    &= -\frac{1}{2} \sum_{\vec{x}}
       \xi^\dagger(x)\;
       \gamma_\mu\,
       \big[ D_{+\mu} - D_{-\mu} \big]\,
       \eta(x) \\
    &\propto \langle \bar{\chi}(t)\, \overleftrightarrow{\Dslash}\, \chi(t) \rangle,
\end{align}

up to lattice discretization effects and the stochastic estimator conventions. In the saved quark 1pt HDF5 file, this quantity can be reconstructed from

\begin{center}
\code{avg/Tmunu/T11}, \code{T22}, \code{T33}, \code{T44}
\end{center}

at the $q = 0$ momentum index. Note that the \code{CHI} datasets are the scalar trace and noise diagnostics, \emph{not} the ringed fermion normalization itself.
...
\subsection{Renormalization Assembly (Analysis Stage)}
\label{sec:renorm_analysis}

The contraction kernel outputs \emph{bare flowed operators}. The final renormalized EMT is assembled in the analysis stage:

\begin{equation}\label{eq:renorm_full}
  T_{\mu\nu}^{\text{ren}}(x)
    = c_g(t)\; O_{\mu\nu}^g(t, x)
    + c_q(t)\; O_{\mu\nu}^q(t, x)
    + c_s(t)\; \big[ O_{\mu\nu}^q(t, x) \big]_{\text{traceless}}
    + \delta_{\mu\nu}\, c_{\text{trace}}(t)\, \Tr\big[O(t,x)\big]
    + \dots
\end{equation}

where:
\begin{itemize}[nosep]
  \item $O_{\mu\nu}^g(t,x)$ is from the gluon 1pt measurement (Sec.~\ref{sec:gluon1pt}).
  \item $O_{\mu\nu}^q(t,x)$ is from the quark 1pt measurement (Sec.~\ref{sec:quark1pt}) for the disconnected piece, and from the connected 3pt measurement (Sec.~\ref{sec:quark_emt_connected}) for the connected piece.
  \item $c_g(t)$, $c_q(t)$ are perturbative matching coefficients computed at small flow time.
  \item Traceless projections and trace anomaly subtraction terms are handled in analysis.
  \item Vacuum subtraction (for the disconnected diagrams) is performed per gauge configuration.
\end{itemize}

The disconnected matrix element is formed in analysis as

\begin{equation}\label{eq:disc_connected}
  C_3^{\text{disc}} = \langle C_2 \, L \rangle - \langle C_2 \rangle \langle L \rangle,
\end{equation}

where $L$ is the quark or gluon loop (1pt) and $C_2$ is the meson two-point function. The quark 1pt and gluon 1pt files provide $L$, while the meson 2pt file provides $C_2$.
...
Quark 1pt     & \code{EMTc/}    & \code{raw/Tmunu\_pervec} & \code{[Nv, 4, 4, Nq, Ns+1, Nt]} \\
              &                  & \code{avg/Tmunu/Tij}    & \code{[Nq, Ns+1, Nt]} \\
Gluon 1pt       & \code{EMTg/}    & \code{Tmunu/Tij}        & \code{[Nq, Ns+1, Nt]} \\
\bottomrule
\end{tabular}
\end{table}
...
The \code{QuarkEMT} and \code{GluonEMT} classes are initialized with a parameter dictionary containing:
...
\end{center}
Typical values: \code{mass = 0.236}, \code{csw = 1.0372}, \code{tol = 1e-15}, \code{maxiter = 300}. The multigrid setup uses a $[[8, 8, 4, 4]]$ block size configuration.

\subsection{Random Number Parameters (Quark 1pt)}
\label{sec:rand_par}

\code{randPara = [n\_vec, n\_zn, randseed]} where \code{n\_vec} is the number of noise vectors, \code{n\_zn} is the $\znum$ order (typically 2), and \code{randseed} is used to seed the random number generator (usually set to the configuration number for reproducibility).
