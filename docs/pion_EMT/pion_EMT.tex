...
\code{raw/Tmunu\_pervec}, without another solve, flow update, derivative, or
gather, and writes the kinetic-only companion
\begin{center}
\code{FlowedQuarkRinged/<lat>.FlowedQuarkRinged.<cfg>.<ama>.<sm>.h5}.
\end{center}
It contains \code{raw/kinetic\_pervec}, source bookkeeping,
\code{avg/kinetic\_spacetime}, and \code{flow\_times}; it does not contain
...
ensemble average of \code{avg/kinetic\_spacetime}; averaging factors formed as
the inverse of each configuration's kinetic value is not equivalent.  The
\code{CHI} datasets remain scalar-trace and noise diagnostics.  The complete
...
explicit finalizer publishes the canonical EMTc and kinetic companion after
