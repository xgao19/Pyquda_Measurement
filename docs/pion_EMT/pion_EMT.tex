...
\code{EMTc/...h5}     & \code{avg/Tmunu/Tij} & \code{[Nq, Ns+1, Nt]} each \\
                       & \code{avg/CHI}      & \code{[2, Nq, Ns+1, Nt]} \\
                       & \code{raw/Tmunu\_pervec} & \code{[Nv, 4, 4, Nq, Ns+1, Nt]} \\
\midrule
\code{EMTg/...h5}     & \code{Tmunu/Tij}   & \code{[Nq, Ns+1, Nt]} each \\
\bottomrule
\end{tabular}
\end{center}
...
$\tau \in [0, N_t - 1]$ for the operator insertion.  Although the 1pt
measurement itself does not involve a source, its file tag shares the same
\code{src} label for bookkeeping, so $t_0$ can be read from it identically.
...
The source position $x_0$ is stored in:
\begin{itemize}[nosep]
  ...
  \item The HDF5 attribute \code{src\_interpolator} or \code{source\_position}
    (convention varies by file type; the file name is the authoritative source).
\end{itemize}
