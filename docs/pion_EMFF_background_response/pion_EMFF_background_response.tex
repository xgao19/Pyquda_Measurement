...
\title{\textbf{Pion EMFF Background-Response Diagnostic}\\[4pt]
\large Finite-Field Energy Shift versus Linearized Response Propagator}
...
This note documents the pion electromagnetic form-factor (EMFF)
background-response diagnostic implemented in
\begin{center}
...
The goal is to study a background-field/Feynman-Hellmann route to pion EMFF
...
\(\Dslash+\lambda\Ocur\).  Instead, the current implementation constructs the
first-order response propagator using ordinary \(\Dslash^{-1}\) inversions.
...
The same source module now also contains a minimal second-order
current-current response building block.  This is intended for future
hadronic-tensor or Compton-amplitude style diagnostics, while the present
production application remains the first-order pion EMFF response.
...
%=============================================================================
\section{Pion EMFF Kinematics}
%=============================================================================

The pion EMFF matrix element is defined by
\begin{equation}
...
interpolator \(\gsink\).  The standard EMFF current is the temporal vector
current,
\begin{equation}
  \Gamma = \gamma_T,
\end{equation}
...
phase convention as the ordinary EMFF code.
...
The helper
\begin{center}
\texttt{build\_local\_current\_inserted\_source(prop\_forward, phase\_q, current\_gamma, tau)}
\end{center}
constructs
\begin{equation}
  \eta(x)
...
\end{equation}
If \texttt{tau} is provided, the source is masked to that global insertion time.
If \texttt{tau} is omitted, all time slices are included.
...
source.  If a list of times \texttt{tau\_list} is given, the code first builds
...
The Perlmutter diagnostic application is
\begin{center}
\texttt{application/EMFF\_pion\_background\_response/perlmutter/}.
\end{center}
...
\begin{enumerate}[label=\arabic*.]
  ...
  response diagnostics without changing the production EMFF contraction.

  \item \textbf{HDF5 schema upgrade.}
  Version-2 output stores file-level kinematics and one
...
The present pion EMFF background-response code is a linearized
