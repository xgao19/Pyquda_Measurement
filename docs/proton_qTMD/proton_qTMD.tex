...
\begin{center}
  \begin{tabular}{l|l}
    ...
    \texttt{pf} & Final nucleon momentum $p_f$ \\
    \texttt{qext} & Momentum transfers for TMD \\
    \texttt{qext\_PDF} & Momentum transfers for PDF \\
    \texttt{plist} & Momentum list (from \texttt{qext}) \\
    \texttt{pilist} & Initial momenta $p_i$ for 2pt \\
    \texttt{width} & Gaussian smearing width \\
    \texttt{boost\_in} & Forward propagator boost smearing \\
    \texttt{boost\_out} & Backward propagator boost smearing \\
    \texttt{pol\_list} & List of polarization projections \\
    \texttt{t\_insert} & Time separation $t_{\text{sep}}$ \\
  \end{tabular}
\end{center}

The production measurement object stores only the forward-line smearing
parameter \texttt{boost\_in}, which is used by its two-point contraction.
The application-level \texttt{boost\_out} remains the sequential/sink
smearing parameter and is passed directly to sequential-source construction;
it is not duplicated as an inactive measurement-class alias.
...
The connected proton qTMD code now has a gauge-invariant staple qTMD path in
addition to the original Coulomb-gauge coordinate-shift qTMD and the straight
PDF operators.  The existing \texttt{application/nucleon\_TMD\_CG} workflow is
kept as the already-tested CG path.  New workflows live in
\texttt{application/nucleon\_TMD/Aurora} and
\texttt{application/nucleon\_TMD/perlmutter}; the Perlmutter version is intended
for CUDA/CuPy smoke tests and production preparation.
...
\begin{enumerate}
  \item \textbf{Forward propagator}: Generate a point source at position
    $(x_0, y_0, z_0, t_0)$, apply boosted smearing, and invert.
  \item \textbf{Two-point contraction}: Call \texttt{contract\_2pt\_TMD} with the
    forward propagator and momentum phases. This computes all 16 gamma channels
    and saves the result as HDF5.
...
\begin{equation}
  \texttt{application/nucleon\_TMD/perlmutter/run\_nucleon\_TMD.sh}.
\end{equation}
