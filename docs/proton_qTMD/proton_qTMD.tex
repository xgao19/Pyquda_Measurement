...
Gamma matrices actually used in the contraction.  The HDF5 attribute
\texttt{gamma\_basis\_schema} has value
\begin{center}
\texttt{pyquda\_bitmask16\_with\_physics\_transform\_v1}.
\end{center}
The exact IDs, matrices, and raw-to-physical transform are stored in
\texttt{gamma\_pyquda\_ids}, \texttt{gamma\_matrices}, and
\texttt{physical\_from\_pyquda}.  The sixteen raw channels are ordered as:
...
\begin{equation}
 ...
 \Gamma_B^{\rm raw}.
\end{equation}
For schema v1 only the \texttt{Y5} and \texttt{T5} diagonal entries are
$-1$, so the transformed axial labels uniformly mean
$\gamma_\mu\gamma_5$.  Raw tensor channels do not contain an additional
factor of $i$; multiply them by $i$ only when using the Hermitian convention
$i[\gamma_\mu,\gamma_\nu]/2$.  The complete schema registry and a portable
HDF5 conversion example are in
...
\begin{enumerate}
  ...
  \item \textbf{Post-inversion $\gamma_5$}: the sequential propagator is conjugated
    ...
    \end{equation}
    Thus complex conjugation converts the source phase
    $e^{+i p_f\cdot(\xvec-\xzerovec)}$ into the effective fixed-sink
    projector $e^{-i p_f\cdot(\xvec-\xzerovec)}$.
\end{enumerate}
...
The Wilson-index columns are $[b_T,b_z,\eta,\text{dir}]$, where
\texttt{dir}=0,1 denotes the \(X,Y\) transverse direction.  PDF data uses the
same schema with \(b_T=0\).  Each file contains one operator, one flavor, and
one polarization, while the complete 16-Gamma scan remains a dataset axis.
The polarization is encoded in both the file tag and HDF5 attributes; it is not
an additional \texttt{corr} axis.
...
CG insertion.  The C2 is polarization independent and remains one shared file
per source.
...
The neutral \texttt{qtmd\_operator\_utils.py} module exposes this through:
\begin{itemize}
  \item \texttt{create\_gi\_qtmd\_wilsonline\_index\_lists(...)} generates
    the shared staple-index lists.
...
\texttt{CG.D.ex}.  Every operator/flavor/polarization file uses the common
dense layout based on $[b_T,b_z,\eta,d_\perp]$.
...
\begin{enumerate}
  ...
  \item \textbf{TMD contractions (CG)}: Loop over Wilson-line indices for
    ...
    Save one dense 16-Gamma file per polarization and flavor.
...
Polarization is included in the sample-log identity, so completing one
polarization does not suppress another.  With all four operators enabled, one
polarization produces one shared C2 file and eight three-point files.
