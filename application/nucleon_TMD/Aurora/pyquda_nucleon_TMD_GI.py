...
parameters = {
    # NOTE:
    "eta": [int(os.environ.get("NUCLEON_TMD_ETA", 12))],
    "b_z": 24,
    "b_T": 24,
...
for ipos, pos in enumerate(src_production):
    ...
    mpi_print(latt_info, f"contract_TMD loop: GI staple links")
    t0_contract = time.time()
    proton_TMDs_down = []
    proton_TMDs_up = []
    staple_links = build_gi_qtmd_staple_links(gauge, W_index_list_GI)

    for iW, WL_indices in enumerate(W_index_list_GI):
        t0 = time.time()
        mpi_print(latt_info, f"TIME PyQUDA: contract GI_qTMD {iW+1}/{len(W_index_list_GI)} {WL_indices}")

        tmd_forward_prop_pyq = Measurement.create_fw_prop_TMD_GI(
            gauge,
            propag,
            WL_indices,
            staple_links=staple_links,
        )
        ...
        mpi_print(latt_info, f"TIME PyQUDA: contract GI_qTMD for U and D {time.time() - t0}s")

    proton_TMDs_down = np.array(proton_TMDs_down)
    proton_TMDs_up = np.array(proton_TMDs_up)
    mpi_print(latt_info, f"contract_GI_qTMD over: proton_TMDs.shape {np.shape(proton_TMDs_down)} {time.time() - t0_contract}s")
    ...
    for i, pol in enumerate(parameters["pol"]):
        ...
        mpi_print(latt_info, f"TIME: save GI_qTMDs for {pol} {time.time() - t0}s")
    mpi_print(latt_info, f"contract_TMD DONE: GI staple links")
