...
def _draw_panel(canvas, rows, panel, y_key, title, ylabel, error_key=None):
    colors = {"zn1024": (0.10, 0.32, 0.70), "hp64x16": (0.80, 0.24, 0.12)}
...
def main():
    ...
    canvas.text(
        306,
        736,
        "Pure stochastic vs stochastic HP16; flow step 1; 1024 matched solves",
        10,
    ...
    labels = {"zn1024": "pure stochastic", "hp64x16": "stochastic HP16"}
    colors = {"zn1024": (0.10, 0.32, 0.70), "hp64x16": (0.80, 0.24, 0.12)}
    for idx, case in enumerate(["zn1024", "hp64x16"]):
        y = 704 - 16 * idx
