...
main=pyquda_nucleon_c2pt_T5_k4.py

# switch to the submit directory
WORKDIR=/lus/flare/projects/StructNGB/xgao/run/l80c80a050/nucleon_TMD_pyquda
cd $WORKDIR
logdir=/lus/flare/projects/StructNGB/xgao/run/l80c80a050/nucleon_TMD_pyquda/log
#rm .cache/*
...
mkdir hostfile
split --lines=${NUM_NODES_PER_MPI} --numeric-suffixes=1 --suffix-length=2 $PBS_NODEFILE hostfile/qTMD_hostfile_stream${stream}_${cfgmin}to${cfgmax}.

k=1
for cfg in $(sed -n "${cfgmin},${cfgmax}p" ${cfglist}); do
  out=${logdir}/qTMD_stream${stream}_${cfg}.out
  err=${logdir}/qTMD_stream${stream}_${cfg}.err
