...
dirac = core.getDirac(latt_info, mass, 1e-4, 5000, xi_0, csw_r, csw_t, multigrid)
...
for ipos, pos in enumerate(src_production):
    ...
    for iW, WL_indices in enumerate(W_index_list_CG_dir0):
        ...
        mpi_print(latt_info, f"TIME PyQUDA TEST 1: contract TMD for D {time.time() - t0}s")
        proton_TMDs_up += [xp.einsum("pgwtzyxjmcf, wtzyxmjfc -> pgwtzyx", sequential_bw_prop_up_contracted_pyq, tmd_forward_prop_dir0.data)]
        mpi_print(latt_info, f"TIME PyQUDA TEST 2: contract TMD for U {time.time() - t0}s")
