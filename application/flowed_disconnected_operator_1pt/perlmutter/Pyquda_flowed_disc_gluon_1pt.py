...
from pyquda_measurement_utils.flowed_disconnected_operator_1pt_vibe_develop import FlowedDisconnectedGluon1pt
from pyquda_measurement_utils.io_corr import get_flowed_disc_gluon_1pt_file_tag
...
parameters = {
    ...
    "flow_epsilon": float(os.environ.get("FLOWED_DISC_FLOW_EPSILON", "0.1")),
    "flow_steps": int(os.environ.get("FLOWED_DISC_FLOW_STEPS", "1")),
}
tag = get_flowed_disc_gluon_1pt_file_tag(data_dir, lat_tag, conf, 0, src_pos, sm_tag)
...
measurement = FlowedDisconnectedGluon1pt(parameters)
gauge.toDevice()
measurement.flowed_1pt(
    gauge,
    tag=os.environ.get("FLOWED_DISC_GLUON_OUT", tag),
)
