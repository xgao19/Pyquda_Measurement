...
parser.add_argument("--stream", type=str, default="b", help="Configuration stream")
...
# Global parameters
data_dir=f"/lus/flare/projects/StructNGB/xgao/run/l80c80a050/TMDWF_pyquda/data_{stream}" # NOTE
lat_tag = "l80c80a050" # NOTE
interpolation = "5" # NOTE, change interpolation operator
sm_tag = f"1HYP_M140_GSRC_W70_k0" # NOTE
...
gauge = io.readNERSCGauge(f"/lus/flare/projects/StructNGB/xgao/ensembles/s8080b7596/gauge_fixed/{stream}/l8080f21b7596m00101m0202{stream}.coulomb.1e-14.{conf}", checksum=False, link_trace=False, plaquette=False)
gauge.hypSmear(1, 0.75, 0.6, 0.3, -1)

mpi_print(latt_info, f"--lat_tag {lat_tag}")
mpi_print(latt_info, f"--sm_tag {sm_tag}")
mpi_print(latt_info, f"--config_stream {stream}")
...
src_production = src_positions[0:16] # take the number of sources needed for this project NOTE
...
for ipos, pos in enumerate(src_production):
    ...
    Measurement.contract_2pt_pion(latt_info, propag_f, propag_b, phases_2pt, tag+f"_src{interpolation}", interpolation)
