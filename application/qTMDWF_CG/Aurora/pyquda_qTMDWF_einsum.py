...
# Global parameters
data_dir="/lus/flare/projects/StructNGB/xgao/run/l64c64a076/TMDWF_pyquda/data" # NOTE
lat_tag = "l64c64a076" # NOTE
interpolation = "5" # NOTE, change interpolation operator
sm_tag = f"1HYP_M140_GSRC_W45_k6_src{interpolation}" # NOTE
...
for ipos, pos in enumerate(src_production):
    ...
    fw_Gsrc = xp.einsum("wtzyxilab, lj -> wtzyxijab", propag_f.data, Gsrc)
    G16_fw_Gsrc = xp.einsum("gim, wtzyxmjab -> gwtzyxijab", pyquda_gamma_ls, fw_Gsrc) 
    ...
    for iW, WL_indices in enumerate(W_index_list_CG_dir0):
        ...
        t0 = time.time()
        temp0 = xp.einsum("ki, wtzyxklab, jl -> wtzyxjiba", G5, tmd_backward_prop_dir0.data.conj(), G5)
        temp1 = xp.einsum("wtzyxjiba, gwtzyxijab -> gwtzyx", temp0, G16_fw_Gsrc)
        temp2 = core.gatherLattice(dnp.asnumpy(xp.einsum("qwtzyx, gwtzyx -> qgt", phases_2pt, temp1)), [2, -1, -1, -1])
    ...
    for iW, WL_indices in enumerate(W_index_list_CG_dir1):
        ...
        t0 = time.time()
        temp0 = xp.einsum("ki, wtzyxklab, jl -> wtzyxjiba", G5, tmd_backward_prop_dir1.data.conj(), G5)
        temp1 = xp.einsum("wtzyxjiba, gwtzyxijab -> gwtzyx", temp0, G16_fw_Gsrc)
        temp2 = core.gatherLattice(dnp.asnumpy(xp.einsum("qwtzyx, gwtzyx -> qgt", phases_2pt, temp1)), [2, -1, -1, -1])
        TMDWF_collect_src5.append(temp2)
