...
from pyquda_measurement_utils.io_corr import save_proton_c2pt_hdf5, get_sample_log_tag, get_c2pt_file_tag, get_qTMD_file_tag, save_qTMD_proton_hdf5_noRoll
...
class QuarkEMT:
    ...
    @classmethod
    def _left_covdev_dst2(cls, U_f: LatticeGauge, seq_bw_prop: LatticePropagator, mu: int):
        """Construct the left-acting derivative on
        ...
        If
            dst2(x) = gamma5 * seq_bw_prop(x)^\dagger * gamma5 ,

        then the left-acting symmetric covariant derivative on dst2 can be built as

            leftD_dst2(x)
            = gamma5 * [ D_mu seq_bw_prop(x) ]^\dagger * gamma5 ,

        where D_mu is the same symmetric covariant derivative used on forward
        propagators.  The returned object has the same propagator-like index
        structure as dst2, namely ``wtzyxadij``.
        """
        D_seq = cls._covdev_sym_prop(U_f, seq_bw_prop, mu)
        D_seq_dag = D_seq.data.conj().transpose(0, 1, 2, 3, 4, 6, 5, 8, 7)
        leftD_dst2 = contract("ab,wtzyxbcij,cd->wtzyxadij", cls.G5, D_seq_dag, cls.G5)
        return leftD_dst2
    ...
    @classmethod
    def get_C3_chi(cls, U_f: LatticeGauge, prop_fw: LatticePropagator,
                seq_bw_prop: LatticePropagator, t0: int, interpolator="5"):
        """Compute the connected meson 3pt function with a local scalar insertion.

        In convention B, we first form

            dst2 = gamma5 * seq_bw_prop^\dagger * gamma5 ,

        where ``seq_bw_prop`` is the sequential propagator through the sink.
        For a scalar insertion, the Dirac structure of the inserted operator is
        the identity, so the local contraction is

            Tr[ dst2(x) * prop_fw(x) * Gamma_src ] ,

        summed over spatial sites and over spin/color, leaving only the time
        dependence t.  Here ``Gamma_src`` is the source interpolator Dirac
        structure (for the pion, Gamma_src = gamma5).

        The tensor contraction

            contract("wtzyxabij, wtzyxbcji, ca->t", dst2, prop_fw.data, Gsrc)

        implements exactly this local trace and spatial sum.  ``gatherLattice``
        then combines MPI ranks, and the result is rolled by ``-t0`` so that the
        source is shifted to time 0.

        Note:
            Depending on the overall meson-contraction convention used elsewhere
            in the code, an additional overall minus sign may be required.
        """
        dst2 = contract(
            "ab,wtzyxbcij,cd->wtzyxadij",
            cls.G5,
            seq_bw_prop.data.conj().transpose(0, 1, 2, 3, 4, 6, 5, 8, 7),
            cls.G5,
        )
        ...
        if interpolator == "5":
            ...
            raise ValueError(f"Unsupported interpolator {interpolator}. Only '5' is supported for now. Add it!")
        
        # TODO check the -sign convention here.
        scalar_t = - contract("wtzyxabij, wtzyxbcji, ca->t", dst2, prop_fw.data, Gsrc)
    ...
    def get_C3_Tmunu_symmetrized(
        ...
        in convention B, with the source interpolator Gamma_src kept explicitly.

        We define the convention-B backward-line object as

            dst2 = gamma5 * seq_bw_prop^\dagger * gamma5 ,

        where ``seq_bw_prop`` is the sequential propagator through the sink.
        The symmetrized EMT insertion is organized as

            + 1/2 Tr[ dst2 * gamma_nu * D_mu(prop_fw) * Gamma_src ]
            - 1/2 Tr[ (left D_mu dst2) * gamma_nu * prop_fw * Gamma_src ].

        Here ``Gamma_src`` is the Dirac structure of the source interpolator.
        For the pion pseudoscalar source, Gamma_src = gamma5.

        The local spin-color trace and local spatial sum are done by einsum,
        leaving only the time dependence t on each MPI rank.  The result is then
        gathered and rolled by ``-t0`` so that the source is shifted to time 0.

        Note:
            Depending on the overall meson-contraction convention used elsewhere
            in the codebase, an additional overall minus sign may be required.
        """
        Nt = U_f.latt_info.global_size[3]
        C3_Tmunu = np.zeros((4, 4, Nt), dtype=np.complex128)

        # convention B: dst2 = gamma5 * seq_bw_prop^\dagger * gamma5
        dst2 = contract(
            "ab,wtzyxbcij,cd->wtzyxadij",
            cls.G5,
            seq_bw_prop.data.conj().transpose(0, 1, 2, 3, 4, 6, 5, 8, 7),
        ...
        # Source interpolator Gamma_src = Gamma_i
        if interpolator == "5":
            Gsrc = gamma.gamma(15)
        else:
            raise ValueError(
                f"Unsupported interpolator {interpolator}. Only '5' is supported for now. Add it!"
            )
        ...
        for mu in range(4):
            ...
            for nu in range(4):
                ...
                # Tr[ dst2 * gamma_nu * D_mu(prop_fw) * Gamma_src ]
                scalar_t = 0.5 * contract(
                    "wtzyxabij,wtzyxbcji,ca->t",
                    dst2,
                    gamma_D_fw,
                    Gsrc,
                )
        ...
        #   - 1/2 Tr[ (left D_mu dst2) * gamma_nu * prop_fw * Gamma_src ]
        for mu in range(4):
            # This helper should act directly on dst2, not reconstruct it from prop_fw.
            leftD_dst2 = cls._left_covdev_dst2(U_f, dst2, mu)
            ...
            for nu in range(4):
                ...
                # Tr[ (left D_mu dst2) * gamma_nu * prop_fw * Gamma_src ]
                scalar_t = -0.5 * contract(
                    "wtzyxabij,wtzyxbcji,ca->t",
                    leftD_dst2,
                    gamma_fw,
    ...
    def contract_2pt_pion(self, latt_info, prop_f, prop_b, phases, tag, interpolator = "5"): 
        ...
        ########### prepare interpolator and momentum phases ###########
        # interpolator = gamma(15) # gamma5 for pion
        if interpolator == "5":
            Gsrc = gamma.gamma(15)
        else:
            raise ValueError(f"Unsupported interpolator {interpolator}. Only '5' is supported for now. Add it!")
        # prepare momentum phases for the sink momentum projection
        phases = _asarray_on_queue(phases, xp, prop_f.data)

        ###################### do contraction ######################
        # source fixed while sink loop over 16 gamma structures, then contract with momentum phases and gather over MPI ranks
        bw_prop = xp.einsum("ij, wtzyxilab, kl -> wtzyxkjba", self.G5, prop_b.data.conj(), self.G5)
    ...
    def connected_3pt(
        ...
        ###################### to get forward and backward propagators ######################
        mpi_print(latt_info, f"src [{x0},{y0},{z0},{t0}]")

        srcFw = source.propagator(latt_info, "point", src_pos)
        srcBw = source.propagator(latt_info, "point", src_pos)

        if self.CG_GaussSmear and self.pos_boost == self.neg_boost:
            t_start = time.time()
            srcFw = boosted_smearing(srcFw, w=self.width, boost=self.pos_boost)
            prop_fw = core.invertPropagator(dirac, srcFw, 1, 0)
            prop_bw = prop_fw.copy()
            mpi_print(latt_info, "source smearing + inversion done for forward propagator (backward propagator is a copy)", time.time() - t_start)
        else:
            t_start = time.time()
            srcFw = boosted_smearing(srcFw, w=self.width, boost=self.pos_boost)
            srcBw = boosted_smearing(srcBw, w=self.width, boost=self.neg_boost)
            prop_fw = core.invertPropagator(dirac, srcFw, 1, 0)
            prop_bw = core.invertPropagator(dirac, srcBw, 1, 0)
            mpi_print(latt_info, "source smearing + inversion done for forward and backward propagators", time.time() - t_start)
        del srcFw, srcBw

        # This copy is used later for the backward line in the 3pt contraction.
        prop_fw_SP = prop_fw.copy()

        ###################### to get two-point correlator ######################
        if self.CG_GaussSmear:
            t_start = time.time()
            prop_fw_SS = boosted_smearing(prop_fw, w=self.width, boost=self.pos_boost)
            prop_bw_SS = boosted_smearing(prop_bw, w=self.width, boost=self.neg_boost)
            mpi_print(latt_info, "sink smearing done for forward and backward propagators", time.time() - t_start)

        p_2pt_xyz = [[-v[0], -v[1], -v[2]] for v in self.pilist]
        phases_2pt = MomentumPhase(latt_info).getPhases(p_2pt_xyz, x0=src_pos)
        tag_2pt = get_c2pt_file_tag(datfile, f"l{Nx}c{Nt}", "0", "ex", src_pos, "smtag")
        self.contract_2pt_pion(latt_info, prop_fw_SS, prop_bw_SS, phases_2pt, tag=tag_2pt, interpolator=src_interpolator)
        del prop_bw_SS, prop_fw

        ###################### prepare gamma list ######################
        # use the first gamma's dtype and device to allocate the container
        xp = _get_xp_from_array(prop_fw_SS.data)
        first_gamma = my_pyquda_gammas[0]
        n_gamma = len(my_pyquda_gammas)
        ...
        for n_ts, t_sep in enumerate(t_separations):
            ...
            t_sink = (t_sep + t0) % Nt
            src_seq_sliced = sequential12(prop_fw_SS, t_sink)

            sink_mom_phase = MomentumPhase(latt_info).getPhases(self.pf, x0=src_pos)
            if sink_interpolator == "5":
                Gsink = gamma.gamma(15)
                G5GsinkG5 = contract("ab,cb,cd->ad", self.G5, Gsink.conj(), self.G5)
            else:
                raise ValueError(f"Unsupported interpolator {sink_interpolator}. Only '5' is supported for now. Add it!")
            src_seq_data = contract("wtzyx, wtzyxijab -> wtzyxijab", sink_mom_phase, src_seq_sliced.data)
            src_seq_data = contract("ik, wtzyxkjab -> wtzyxijab", G5GsinkG5, src_seq_data)
            ...
            dirac.loadGauge(U)
            seq_prop_bw = core.invertPropagator(dirac, src_seq, 1, 0)
            del src_seq, src_seq_data

            # TODO add momentum transfer for the 3pt function.
            p_3pt_xyz = [[-v[0], -v[1], -v[2]] for v in self.qlist]
            phases_3pt = MomentumPhase(latt_info).getPhases(p_3pt_xyz, x0=src_pos)

            prop_fw_SP_3pt = prop_fw_SP.copy()
            U_f = U.copy()
            U_f.setAntiPeriodicT()

            for step in range(Nsteps + 1):
                mpi_print(latt_info, f"contraction for step {step}")
                C3_chi[n_ts, step] += self.get_C3_chi(U_f, prop_fw_SP_3pt, seq_prop_bw, t0, interpolator = src_interpolator)
                C3_Tmunu[n_ts, step] += self.get_C3_Tmunu_symmetrized(U_f, prop_fw_SP_3pt, seq_prop_bw, t0, interpolator=src_interpolator)

                if Nsteps > 0 and step == 1:
                    # First step is subdivided into 10 smaller flow steps to better preserve the initial condition.
                    prop_fw_SP_3pt, seq_prop_bw = self._flow_two_props_pyquda(U_f, prop_fw_SP_3pt, seq_prop_bw, stepsize/10, Nsteps=10, flow_type=self.flow_type)
                elif Nsteps > 0 and step < Nsteps:
                    # Advance both propagators and the gauge field to the next
                    # flow time.
                    prop_fw_SP_3pt, seq_prop_bw = self._flow_two_props_pyquda(U_f, prop_fw_SP_3pt, seq_prop_bw, stepsize, Nsteps=1, flow_type=self.flow_type)

            del U_f, prop_fw_SP_3pt, seq_prop_bw

        np.save(f"{datfile}/C2_spin{sink_interpolator}_HYP_SS.npy", C2)
        np.save(f"{datfile}/C3_chi_spin{sink_interpolator}_HYP_SS.npy", C3_chi)
        np.save(f"{datfile}/C3_Tmunu_spin{sink_interpolator}_HYP_SS.npy", C3_Tmunu)
